...
export default function BookPolicyManagement() {
  ...
  const fetchPolicies = async () => {
    try {
      setLoading(true);
      const response = await api.get('/book-policies');
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingPolicy) {
        await api.put(`/book-policies/${editingPolicy.id}`, payload);
        setSuccess('Policy updated successfully');
      } else {
        await api.post('/book-policies', payload);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/book-policies/${id}`);
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
              ...
              Book Incentive Policies
            </h1>
            <p className="mt-2 text-gray-600">
              Manage incentive structures for book publications
            </p>
          </div>
          <button
      ...
      <div className="space-y-4">
        {policies.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <p className="text-gray-600 mb-4">Create your first book incentive policy to get started</p>
            <button
              onClick={() => handleOpenModal()}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, publicationType: 'book' })}
